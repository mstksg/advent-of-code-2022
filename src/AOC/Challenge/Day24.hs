...
import           AOC.Common.Point (Point, Dir(..), parseAsciiMap, cardinalNeighbs, dirPoint, boundingBox, mannDist)
  ...
  where
    ...
    expander (MS t p) = M.fromAscList [(MS (t + 1) p', 1) | p' <- S.toList cands ]
      where
        ...
        cands = (S.fromList (p:cardinalNeighbs p) `S.intersection` validPos)
                `S.difference` bs
